...
const BODY_FONT = "ui-sans-serif, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
...
function hoursLabel(minutes) { const h = minutes / 60; return `${h % 1 === 0 ? h.toFixed(0) : h.toFixed(1)}h`; }
function last7Days() {
  const days = [];
  for (let i = 6; i >= 0; i--) {
...
function dayShortLabel(dateStr) { return new Date(dateStr).toLocaleDateString(undefined, { weekday: "narrow" }); }
